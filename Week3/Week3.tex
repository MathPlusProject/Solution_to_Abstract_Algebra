...
教材的符号有不清晰的地方, 主要是素理想那一块, 我进行了修正, 用$\subseteq$表示子集, $\subsetneq$表示真子集.
...
注: 零理想的根$\sqrt{\{0\}} = \{x \in R \mid \exists n \in \mathbb{N}, x^n = 0\}$是所有幂零元(nilpotent)组成的理想, 叫做$R$的幂零根(nilradical), 一般记作$\mathfrak{N}(R)$. 可以证明$\mathfrak{N}(R) = \bigcap_{\mathfrak{p} \text{ 是素理想}} \mathfrak{p}$.
\end{solution}
